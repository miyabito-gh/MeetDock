...
| `prepare_dropped_files` | `{ paths: string[] }` | `{ candidates: { name, path, target_type: "file" | "folder" }[] }` |
...
- `prepare_dropped_files`はネイティブDnD入力専用であり、Rustが存在する通常ファイルまたはフォルダをcanonicalizeし、Windows拡張プレフィックスを通常のドライブ／UNC表記へ戻した種別付き登録候補を返す。フォルダ配下は展開しない。候補は確認後にのみdraftへ追加し、起動系IPCへは渡さない。
