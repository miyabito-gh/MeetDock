...
## 13. 2026-09-20 設計差分監査と次回実装対象

ソースと設計書を再照合した結果、次の製品実装差分を確認した。詳細な作業順、対象ファイル、直近の修正済み事項は`NEXT_SESSION_PROMPT.md`を正とする。

1. **PID/HWNDセッション追跡は実装済み**。Explorer限定ではなく、MeetDockが起動した外部プロセスを対象に、PID/HWND/プロセス開始時刻をセッション中だけ保持し、終了・無効化・PID再利用を検証して破棄する。委譲・複数候補・HWND未取得は追跡不能として扱う。
2. Office COM/ROTによるExcel・Word文書の絶対パス特定が未実装。
3. Restart Managerによる補助判定が未実装。
4. タイトル照合にPID・実行ファイル名を組み合わせる多層照合が未実装。現在は可視ウィンドウのタイトル照合のみ。
5. 診断ログのローテーションと利用者による明示エクスポートが未実装。
6. 二重起動時、既存ウィンドウの前面化拒否に対するタスクバー通知が未実装。
7. Windows/PDF/UNC/原子的置換およびUI-03の実機受入が未合格。

一括起動の250ms間隔・未着手項目キャンセル・結果明細、DnD事前検証・確認画面、PDF外部起動導線、検索時グループ表示、PDFページ／倍率／Esc、最終同期時刻は実装済み。以前の差分一覧は更新済み内容と混同しないこと。
...
設定保存については、保存後の再起動による設定復元と、編集中に`settings.json`の`revision`を外部変更した場合の競合検出を実機確認済み。競合時は「別の変更と競合しました。編集内容は保持されています。」と表示され、編集内容は保持された。txt、PDF、フォルダ、HTMLファイル、設定復元、revision競合は現時点で検証済み。未確認環境では、まず現行実装を検証し、実際に問題が発生した場合だけ追加実装を検討する。
